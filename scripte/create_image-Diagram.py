...
def updateAxes(fig):
    ...
    fig.update_xaxes(
        ...
        tickangle=0  # Optional: Labels schräg setzen, um Platz zu sparen
    )
...
def rename_name_column(name):
    ...
    return name.split('city_filter_colourChannel_')[1]

def buildDiagram(path):
    title = "city_filter_colourChannel"      #Ändern
    df_content = fromMarkdownToDataFrame(path, 4)
    df_content['name'] =  df_content['name'].apply(rename_name_column)
    
    df_semantic = fromMarkdownToDataFrame(path, 3)
    df_semantic['name'] =  df_semantic['name'].apply(rename_name_column)

    fig_content = create_Bar_Diagram(df_content, "Content")
    updateAxes(fig_content)
    updateLayout(fig_content, title)
    createJPEG(fig_content, title.split("city_")[1]+"_content-code", path)    #Ändern
    fig_content.show()

    fig_semantic = create_Bar_Diagram(df_semantic, "Semantic")
    updateAxes(fig_semantic)
    updateLayout(fig_semantic, title)
    createJPEG(fig_semantic, title.split("city_")[1]+"_semantic-code", path)    #Ändern 
    fig_semantic.show()

def main():
    path = "Desktop/bachelorarbeit/twinspectTest/results/image/test"
    buildDiagram(path)
