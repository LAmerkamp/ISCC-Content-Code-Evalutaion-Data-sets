...
def fromMarkdownToDataFrame(path):
    ...
    for comparison_file in comparison_files:
        ...
        for line in lines[4:]:
            ...
            if re.search(r"page|textblock|sentence|word|character", filename):
    ...
    df_character = pd.DataFrame([entry for entry in data if "character" in entry['name']])
...
def create_Bar_Diagram(df):
    return px.bar(
        ...
        labels={
            'name': 'Modification',
            'contentSim': 'Semantic - Similarity',
            'resource': 'Text size:'
        },
        color_discrete_map={
            '16784Words': '#0072B2',
...
def getTitle(df):
    targets = ['word', 'character', 'sentence', 'textblock', 'page']
    name = df['name']
...
def buildDiagrams(path):
    ...
    for target in targets:
        ...
        for group_name, group_df in df.items():
            ...
            updateAxes(fig)
            updateLayout(fig, title)
            createJPEG(fig, f"{group_name}_{target}_semantic-code", path)    
            #fig.show() 

def main():
    path = "Desktop/bachelorarbeit/twinspectTest/results/text/txt"
    buildDiagrams(path)
